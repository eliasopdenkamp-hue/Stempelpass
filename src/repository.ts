...
export class CardRepository {
   ...
   */
  async configurePilot(tenantId:string, actorUserId:string, input:{planCode:'up_to_500'|'up_to_1000';cardTitle:string;cardText:string;primaryColor:string;secondaryColor:string;iconAssetId?:string;logoAssetId?:string;logoUrl?:string;stampsRequired:number;rewardTitle:string;rewardDescription:string}) {
    ...
    return this.transaction(tenantId, async db => {
      ...
      await db.query(`insert into tenant_branding(tenant_id,card_title,card_text,primary_color,secondary_color,icon_asset_id,logo_asset_id,logo_url,version) values($1,$2,$3,$4,$5,$6,$7,$8,1) on conflict(tenant_id) do update set card_title=excluded.card_title,card_text=excluded.card_text,primary_color=excluded.primary_color,secondary_color=excluded.secondary_color,icon_asset_id=excluded.icon_asset_id,logo_asset_id=excluded.logo_asset_id,logo_url=excluded.logo_url,version=tenant_branding.version+1,updated_at=now()`,[tenantId,input.cardTitle.trim(),input.cardText.trim(),input.primaryColor,input.secondaryColor,input.iconAssetId||null,input.logoAssetId||null,input.logoUrl?.trim()||null]);
  ...
  async staffDashboard(tenantId:string):Promise<StaffDashboardData> { return this.transaction(tenantId, async db => {
    ...
    const b=(await db.query<Branding>('select card_title as "cardTitle",card_text as "cardText",primary_color as "primaryColor",secondary_color as "secondaryColor",logo_url as "logoUrl",version from tenant_branding where tenant_id=$1',[tenantId])).rows[0] ?? null;
