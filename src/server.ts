...
if(parts[3]==='cards'&&parts.length===4&&req.method==='POST'){const body=await req.json() as {customerId?:string,ruleId?:string};if(!body.customerId||!body.ruleId)throw new Error('CARD_FIELDS_REQUIRED');const card=await repository!.createCard(tenantId,body.customerId,body.ruleId,hashToken(randomToken()));return json(toCreateCardResponse(card),201,id);}
